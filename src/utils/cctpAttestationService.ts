...
const IRIS_ATTESTATION_API_URL = 'https://iris-api.circle.com';
...
export const getAttestation = async (messageHash: string): Promise<Attestation | null> => {
  try {
    const response = await axiosInstance.get<AttestationResponse>(`/${messageHash}`);

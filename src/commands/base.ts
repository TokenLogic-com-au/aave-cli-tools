...
function getMessenger() {
  ...
  const L1_CHAIN_ID = 1;
  const L2_CHAIN_ID = 8453;

  const l1_provider = new providers.StaticJsonRpcProvider(process.env.RPC_MAINNET);
  const l2_provider = new providers.StaticJsonRpcProvider(process.env.RPC_BASE);
...
export function addCommand(program: Command) {
  program
    .command('base-prove-message')
    .description('prove base transaction to confirm bridge')
    .argument('<tx_hash>')
    .argument('<log_index>')
  ...
  program
    .command('base-finalize-bridge')
    .description('finalizes base transaction to bridge')
    .argument('<tx_hash>')
    .argument('<log_index>')
    .action(async (tx_hash, log_index) => {
      const messenger = getMessenger();
